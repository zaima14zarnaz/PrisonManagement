...
$firstname = $lastname = $specialty = $gender = $phone = $address = $designation = $name = "";
$age = $salary = 0;
...
if(isset($_GET['go']))
{
    ...
    if(!empty($po)) $designation = $po;
    if(!empty($ge)) $gender = $ge;
    if(!empty($sp)) $specialty = $sp;

    $name = $firstname." ".$lastname;

    $update = "UPDATE employee SET name='$name',age=$age,phone_no='$phone',address='$address',post='$designation',gender='$gender',salary='$salary',specialty='$specialty' WHERE eid=".$_SESSION['id'];
...
<html>
    ...
    <form method="get" action="mupform.blade.php">
  ...
  <div class="form-row">
    <div class="form-group col-md-6">
      ...
      <input type="text" class="form-control" name="post" placeholder="<?php echo $designation;?>">
    </div>
    <div class="form-group col-md-6">
      <label>Specialty</label>
      <input type="text" class="form-control" name="specialty" placeholder="<?php echo $specialty;?>">
    </div>
  </div>
  <div class="form-group" style="width:370px;">
    <label>Salary</label>
