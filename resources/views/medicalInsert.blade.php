...
if(isset($_GET['go']))
{
    ...
    if(empty($name) or empty($start) or empty($end) or empty($salary) or empty($address) or empty($age) or empty($gender) or empty($post) or empty($phone) or empty($specilaty));
    $db = mysqli_connect("localhost","root","","PrisonManagement");
    
    $getUser = "INSERT INTO employee(name,age,start,end,gender,phone_no,address,type,post,specialty,salary) VALUES('$name','$age',$start,$end,'$gender','$phone','$address','medical','$post','$specialty','$salary')";
...
<html>
    ...
    <form method="get" action="medicalInsert.blade.php">
