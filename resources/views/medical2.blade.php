...
<nav class="navbar navbar-expand-lg navbar-light fixed-top py-3" id="mainNav" style="background-color:orange;color:white">
  ...
  <div style="margin-top:70px;"></div>
    ...
    <form action="medical2.blade.php" method="get">
        <div class="form">
  ...
  if(isset($_GET['go'])){
    ...
    $time=$_GET['start'].":".$_GET['startm'];
   
    
        $group="SELECT COUNT(eid) as num, post,type,start,end,STRCMP('$time', start) as one,STRCMP(end,'$time') as two FROM employee GROUP BY post HAVING type='medical' AND STRCMP('$time', start)>=0 AND STRCMP(end,'$time')<=0";
